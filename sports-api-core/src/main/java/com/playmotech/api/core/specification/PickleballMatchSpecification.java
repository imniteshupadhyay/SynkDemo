...
import com.playmotech.api.core.dao_postgres.PickleballMatch_;
...
public class PickleballMatchSpecification implements Specification<PickleballMatch> {
    ...
    public PickleballMatchSpecification(GenericFilter filter, MatchStatus matchStatus) {
        this.filter = filter;
    ...
    public PickleballMatchSpecification(GenericFilter filter, List<MatchStatus> matchStatuses) {
        this.filter = filter;
    ...
    public PickleballMatchSpecification(GenericFilter filter) {
        this.filter = filter;
        this.matchStatus = null;
        this.matchStatuses = null;
    }
    ...
    private static boolean isValidOrderByColumn(String column) {
        return PickleballMatch_.START_TIME.equals(column)
                || PickleballMatch_.END_TIME.equals(column)
                || PickleballMatch_.MATCH_STATUS.equals(column)
                || PickleballMatch_.CREATED_AT_TIMESTAMP_UTC.equals(column)
                || PickleballMatch_.UPDATED_AT_TIMESTAMP_UTC.equals(column)
                || PickleballMatch_.SCHEDULED_START_TIME.equals(column)
                || "courtName".equals(column); // For court.courtName
    }

    private void addOrderBy(Root<PickleballMatch> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        if (isStringNotNullAndBlank.test(filter.getOrderBy())) {
            ...
            if (isValidOrderByColumn(orderBy)) {
                ...
                order = filter.isAscending() ? builder.asc(root.get(PickleballMatch_.CREATED_AT_TIMESTAMP_UTC))
                        : builder.desc(root.get(PickleballMatch_.CREATED_AT_TIMESTAMP_UTC));
            }

            query.orderBy(order);
        } else {
            // Default sorting when no orderBy is specified
            query.orderBy(builder.desc(root.get(PickleballMatch_.CREATED_AT_TIMESTAMP_UTC)));
        }
    }

    private static <T> void addEqualityPredicate(Root<PickleballMatch> root, CriteriaBuilder builder,
    ...
    private static <T> void addInPredicate(Root<PickleballMatch> root, CriteriaBuilder builder,
    ...
    private void addSearchLikePredicate(Root<PickleballMatch> root, CriteriaBuilder builder,
    ...
    private void addActiveFilter(Root<PickleballMatch> root, CriteriaBuilder builder, List<Predicate> predicates) {
        // Always filter out inactive matches
        predicates.add(builder.isFalse(root.get("inactive")));
    }

    private void addAcademyFilter(Root<PickleballMatch> root, CriteriaBuilder builder, List<Predicate> predicates) {
    ...
    private void addTournamentFilter(Root<PickleballMatch> root, CriteriaBuilder builder, List<Predicate> predicates) {
    ...
    private void addMatchStatusFilter(Root<PickleballMatch> root, CriteriaBuilder builder, List<Predicate> predicates) {
        if (matchStatus != null) {
    ...
    @Override
    public Predicate toPredicate(Root<PickleballMatch> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
