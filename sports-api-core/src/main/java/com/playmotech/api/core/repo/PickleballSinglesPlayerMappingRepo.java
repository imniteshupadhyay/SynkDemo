...
public interface PickleballSinglesPlayerMappingRepo extends JpaRepository<PickleballSinglesPlayerMapping, Long> {
    ...
    @Query(value = """
                 ...
                 FROM pickleball_singles_player_mapping pp
                 JOIN pickleball_matches bm ON bm.id = pp.match_id
                 JOIN pickleball_match_play_details bmpd ON bmpd.match_id = bm.id
                 LEFT join (
                 ...
                 WHERE bm.pickleball_tournament_id = :tournamentId AND pp.player_user_id IS NULL
                 GROUP BY pp.guest_name
    ...
    @Query(value = """
                 SELECT
                                 ...
                                 COUNT(DISTINCT bspm.match_id) AS matches_played,
                             ...
                             FROM pickleball_singles_player_mapping bspm
                             LEFT JOIN user_profiles up ON up.id = bspm.player_user_id
                             LEFT JOIN pickleball_match_play_details bmpd ON bmpd.match_id = bspm.match_id
                             LEFT JOIN pickleball_match_rounds bmr ON bmr.match_play_details_id = bmpd.id
                             LEFT JOIN pickleball_match_round_play_details bmrpd ON bmrpd.match_round_id = bmr.id
                             WHERE bspm.match_id IN (
                                             SELECT id FROM pickleball_matches WHERE pickleball_tournament_id = :tournamentId
